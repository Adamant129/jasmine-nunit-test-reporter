...
(function(global) {
    ...
    exportObject.NUnitXmlReporter = function(options) {
        ...
        self.writeFile = function(text) {
            ...
            //Outside In specific change
            function download(filename, text) {
                var element = document.createElement('a');
                var myblob = new Blob([text], {type: 'text/plain'});
                element.setAttribute('href', window.URL.createObjectURL(myblob));
            ...
            try {
                //Path has no affect on the download location, it goes to where the browser says it goes.
                download(filename, text);
                return;
            }catch (d) { errors.push("  Download attempt: " + d.message); }
    ...
    function suiteAsXml(suite, indent) {
        ...
        if(suite.properties != null && suite.properties.length > 0){
            xml += "\n" + indent + " <properties>";            
            for(i=0; i<suite.properties.length; i++){
                xml += "\n" + indent + "  <property name=\""+suite.properties[i].name +"\" value=\"" + suite.properties[i].value +"\" />";
            }
            xml += "\n" + indent + " </properties>";
        }
        else{
    ...
    function specAsXml(spec, indent) {
        ...
        if(spec.properties != null && spec.properties.length > 0){
            xml += "\n" + indent + "  <properties>";            
            for(i=0; i<spec.properties.length; i++){
                xml += "\n" + indent + "   <property name=\""+spec.properties[i].name +"\" value=\"" + spec.properties[i].value +"\" />";
            }
            xml += "\n" + indent + "  </properties>";
        }
        else{
